...
if(isset($_POST["newCookie"])){
	setcookie("PHPSESSID", str_replace('.', '', $_SERVER['REMOTE_ADDR']) . generateRandomString(), time()+3600, "", "KVH.nl");
}
...
if(isset($_POST["recieveAuthorizedMail"])){
  ...
  $url = $_POST["url"];
    ...
    $result = getQueryResult($dbConnection, $query);
      $headers = 'From: KVH <admin@KVH.nl>' . "\r\n";
      $headers .='X-Mailer: PHP/' . phpversion();
      $headers .= "MIME-Version: 1.0\r\n";
      $headers .= "Content-type: text/html; charset=iso-8859-1\r\n";
  $message = " Hallo $name, <br><br> U heeft een account aangemaakt op de 'KVH' website. <br>De administrator heeft uw toegang gegeven zodat u kan inloggen. <br><br> U kunt inloggen via: $url <br> Succes! <br><br> Met vriendelijk groet, <br> Administrator - KVH";
  mail($email, "Toegang - KVH", $message, $headers);
}
...
if(isset($_POST["userExistAndSendMail"])){
    ...
    if (is_null($resultEmail) || empty($resultEmail) || !isset($resultEmail) || $resultEmail === $emptyString) {
        ...
        $message = " Hallo, <br><br> Hierbij ontvangt u de code: $generate <br> Op $url kunt u de code invoeren om toegang te krijgen om een nieuw wachtwoord te kunnen maken. <br><br><br> Met vriendelijk groet, <br> Administrator - KVH";
        mail($email, "Nieuw wachtwoord - KVH", $message, $headers);
        echo $resultEmail;
      }
}
...
function sendMailForActivation($name, $insertion, $lastName, $email, $url){
    ...
    if (is_null($insertion) || empty($insertion) || !isset($insertion)) {
      ...
      $headers .= "MIME-Version: 1.0\r\n";
      $headers .= "Content-type: text/html; charset=iso-8859-1\r\n";
      $message = "Hallo, <br><br> '$fullName' wil graag een account aanmaken. <br><br> Ga naar het beheerpaneel via $url om het account/emailadres van $email te activeren. <br><br><br> Met vriendelijke groet, <br> KVH";
      mail($emailReciever, "$fullName wil zich graag bij KVH aanmelden", $message, $headers);
...
if(isset($_POST["commentOnMarker"])){
  ...
  if($userName != $viewerName){
    ...
    $headers .= "MIME-Version: 1.0\r\n";
    $headers .= "Content-type: text/html; charset=iso-8859-1\r\n";
    $message = " Hallo '$userName', <br><br>'$viewerName' heeft een nieuwe reactie geplaatst bij uw video met de titel '$videoTitle'.<br> U kunt de video bekijken op www.KVH.nl <br><br><br> Met vriendelijk groet, <br> Administrator - KVH";
    mail($email, "Reactie - KVH", $message, $headers);
  }
  return makeJSONofResults($result);
}
...
if(isset($_POST["addSharedUser"])){
  ...
  $headers .= "MIME-Version: 1.0\r\n";
  $headers .= "Content-type: text/html; charset=iso-8859-1\r\n";
  $message = " Hallo '$viewerName', <br><br>U bent aan mijn netwerk toegevoegd.<br> U kunt mijn video's bekijken op www.KVH.nl <br><br><br> Met vriendelijk groet, <br> '$userName' - KVH";
  mail($email, "Netwerk - KVH", $message, $headers);
}
